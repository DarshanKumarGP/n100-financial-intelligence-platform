    ...
    pl = pd.DataFrame(
        {
            ...
            "opm_percentage": [50.0],  # real OPM=20%, source says 50%
        }
    )
...
def test_dq08_ticker_too_short_rejected():
    df = pd.DataFrame(
        {"company_id": ["A"], "year": ["2023-03"]}
    )  # 1 char, below the 2-char minimum
