...
def load_all_company_data(conn):
    ...
    for company_id, group in fr.groupby("company_id"):
        ...
        if pd.notna(latest.get("return_on_equity_pct")) and latest["return_on_equity_pct"] > 500:
...
def pro_02_fcf_positive_5yr(history):
    ...
    if last5.notna().all() and (last5 > 0).all():
        return 80, "Strong free cash flow generation over 5 years signals healthy business fundamentals"
    return None
...
def pro_03_debt_free(latest):
    ...
    if pd.notna(de) and de < 0.01:
        return 85, "Debt-free balance sheet provides financial flexibility and eliminates interest burden"
    return None
...
def pro_04_revenue_cagr_15(latest):
    ...
    if pd.notna(v) and v > 15:
        return clamp_confidence(70 + (v - 15)), "Revenue growing at above 15% CAGR over 5 years reflects strong business momentum"
    return None
...
def pro_05_opm_25(latest):
    ...
    if pd.notna(v) and v > 25:
        return clamp_confidence(70 + (v - 25) / 2), "Operating profit margin above 25% indicates strong pricing power and cost discipline"
    return None
...
def pro_06_pat_cagr_20(latest):
    ...
    if pd.notna(v) and v > 20:
        return clamp_confidence(70 + (v - 20)), "Net profit compounding at above 20% over 5 years creates significant shareholder value"
    return None
...
def pro_07_icr_high(latest):
    ...
    if pd.notna(icr) and icr > 10:
        return clamp_confidence(70 + (icr - 10)), "Very high interest coverage ratio reflects negligible financial stress from debt servicing"
    return None
...
def pro_08_dividend_yield(latest, dividend_yield):
    ...
    if pd.notna(dividend_yield) and dividend_yield > 2 and pd.notna(fcf) and fcf > 0:
        return clamp_confidence(70 + (dividend_yield - 2) * 3), "Consistent dividend yield above 2% backed by positive free cash flow"
    return None
...
def pro_09_eps_cagr_15(latest):
    ...
    if pd.notna(v) and v > 15:
        return clamp_confidence(70 + (v - 15)), "Earnings per share growing above 15% CAGR indicates strong earnings quality and compounding"
    return None
...
def pro_11_operating_leverage(latest):
    ...
    if pd.notna(rev) and pd.notna(pat) and pat > rev and rev > 0:
        return clamp_confidence(65 + (pat - rev)), "Revenue growing slower than profits shows improving operating leverage and scale benefits"
    return None
...
def pro_12_assets_growing_debt_declining(bs_history):
    ...
    if (last3["total_assets"].is_monotonic_increasing and
        last3["borrowings"].is_monotonic_decreasing):
        return 78, "Growing asset base funded by internal accruals reflects self-sustaining growth"
    return None
...
def pro_13_fallback_stable_de(latest):
    ...
    if pd.notna(de) and 0 <= de <= 1.0:
        return 62, "Manageable debt-to-equity ratio indicates the balance sheet is not under financial stress"
    return None
...
def con_01_high_de(latest, is_financials):
    ...
    if pd.notna(de) and de > 2.0:
        return clamp_confidence(70 + (de - 2) * 5), f"Debt-to-equity ratio of {de:.1f} is elevated for a non-financial company and warrants monitoring"
    return None
...
def con_02_fcf_negative_3yr(history):
    ...
    if last3.notna().all() and (last3 < 0).all():
        return 80, "Free cash flow negative for 3 consecutive years raises concern about cash generation quality"
    return None
...
def con_03_opm_declining(history):
    ...
    if last3.notna().all() and last3.is_monotonic_decreasing:
        return 72, "Operating margins declining for 3 consecutive years suggest pricing or cost pressure"
    return None
...
def con_06_low_icr(latest):
    if latest.get("icr_label") == "Debt Free":
        return None
    icr = latest.get("interest_coverage")
    if pd.notna(icr) and icr < 1.5:
        return clamp_confidence(85 - icr * 5), "Interest coverage ratio below 1.5x indicates the company is at risk of not meeting its debt obligations"
    return None
...
def con_07_high_payout(latest):
    ...
    if pd.notna(v) and v > 100:
        return clamp_confidence(70 + (v - 100) / 5), "Dividend payout ratio above 100% means the company is paying dividends from reserves, which is unsustainable"
    return None
...
def con_08_de_rising(history):
    ...
    if last3.notna().all() and last3.is_monotonic_increasing and last3.iloc[-1] > last3.iloc[0]:
...
def con_10_low_roce(latest, is_outlier):
    ...
    if pd.notna(v) and 0 <= v < 10:
        return clamp_confidence(85 - v), "Return on capital employed below 10% suggests the business is not generating sufficient returns on invested capital"
    return None
...
def con_12_low_revenue_cagr(latest):
    ...
    if pd.notna(v) and v < 5:
        return clamp_confidence(75 - v * 3), "Revenue growing at below 5% over 5 years lags inflation and suggests limited business momentum"
    return None
...
def con_13_fallback_relative_pe(latest, sector_median_pe):
    ...
    if pd.notna(pe) and pd.notna(sector_median_pe) and sector_median_pe > 0 and pe > sector_median_pe:
        return 65, "Stock trades at a premium to sector median P/E, reflecting high market expectations already priced in"
    return None
...
def con_14_fallback_low_dividend(dividend_yield):
    ...
    if pd.notna(dividend_yield) and dividend_yield < 0.5:
        return 62, "Low dividend yield means limited direct cash returns to shareholders relative to reinvestment-focused strategy"
    return None
...
def con_15_fallback_low_fcf_conversion(latest):
    ...
    if pd.notna(v) and v < 100:
        return 63, "Free cash flow conversion below 100% of operating profit means some reported profit is not yet converting to cash"
    return None
...
def con_16_fallback_low_cfo_quality(latest):
    ...
    if pd.notna(v) and v < 0.5:
        return 61, "Cash flow from operations relative to net profit over the past 5 years is below a healthy conversion threshold, indicating some earnings quality risk"
    return None


def main():
    conn = sqlite3.connect(DB_PATH)
    fr, pl, bs, sectors, mc_latest, outliers = load_all_company_data(conn)
    ...
    sector_median_pe = sector_pe_df.groupby("broad_sector")["pe_ratio"].median().to_dict()
    ...
    for company_id in companies:
        ...
        pe_ratio_val = div_row["pe_ratio"].iloc[0] if len(div_row) else None  # pe_ratio lives in market_cap, not financial_ratios
        ...
        rule_results.append(("pro", "P08", pro_08_dividend_yield(latest, dividend_yield)))
        ...
        rule_results.append(("pro", "P12", pro_12_assets_growing_debt_declining(bs_history)))
        ...
        rule_results.append(("con", "C06", con_06_low_icr(latest)))
        ...
        # C11: Net Debt > 3x EBITDA (operating_profit as EBITDA proxy)
        net_debt = latest.get("net_debt_cr")
        op_profit = pl_latest.get("operating_profit") if len(pl_history) else None
        if pd.notna(net_debt) and pd.notna(op_profit) and op_profit > 0 and net_debt > 3 * op_profit:
            conf = clamp_confidence(70 + (net_debt / op_profit - 3) * 5)
            rule_results.append(("con", "C11", (conf, "Net debt exceeding 3 times EBITDA is a high leverage ratio and limits financial flexibility")))
        else:
            rule_results.append(("con", "C11", None))
        ...
        if not cons_so_far:
            ...
            fallback_id, fallback = "C13_fallback", con_13_fallback_relative_pe({"pe_ratio": pe_ratio_val}, median_pe)
            if fallback is None:
                fallback_id, fallback = "C14_fallback", con_14_fallback_low_dividend(dividend_yield)
            if fallback is None:
                fallback_id, fallback = "C15_fallback", con_15_fallback_low_fcf_conversion(latest)
            if fallback is None:
                fallback_id, fallback = "C16_fallback", con_16_fallback_low_cfo_quality(latest)
        ...
        for rule_type, rule_id, result in rule_results:
            if result is not None:
                ...
                if confidence > 60:
                    all_records.append({
                        "company_id": company_id, "type": rule_type,
                        "rule_id": rule_id, "text": text,
    ...
    print(f"Distinct companies with at least one entry: {output_df['company_id'].nunique()}")
