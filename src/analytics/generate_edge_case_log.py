...
def categorize_anomaly(
    ...
    if (
        ...
        and equity_plus_reserves is not None
        and equity_plus_reserves > 0
    ):
...
def main():
    """CLI entry point: cross-check computed ROE/ROCE against source values and log categorized anomalies to output/ratio_edge_cases.log."""
    ...
    for (
        ...
        if (
            ...
            and abs(computed_roce - source_roce) > 5
        ):
        ...
        if (
            ...
            and abs(computed_roe - source_roe) > 5
        ):
            category, note = categorize_anomaly(
                company_id,
                computed_roe,
    ...
    with open(LOG_PATH, "w", encoding="utf-8") as f:
        f.write("N100 Financial Intelligence Platform -- Ratio Edge Case Log\n")
        f.write(f"Generated: {datetime.now(timezone.utc).isoformat()}\n")
        f.write(
            f"Total anomalies found (>5% diff, computed vs source): {len(entries)}\n"
        )
        f.write("=" * 80 + "\n\n")

        for e in entries:
            f.write(
                f"[{e['category'].upper()}] {e['company_id']} -- {e['metric']} ({e['year']})\n"
            )
            f.write(
                f"  Computed: {e['computed']:.2f}%  |  Source: {e['source']:.2f}%  |  Diff: {e['diff']:.2f}pp\n"
            )
        ...
        f.write(
            "  is unreliable for 21 companies (155/234 flagged rows in Financials\n"
        )
        ...
        f.write(
            "  across every year. P&L-based ratios (NPM, OPM) remain populated.\n\n"
        )
