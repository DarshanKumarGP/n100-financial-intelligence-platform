...
PRESET_THRESHOLDS = {
    ...
    "Turnaround Watch": {
        "revenue_cagr_5yr": (
            lambda v, t: True,
            None,
        ),  # revenue_cagr_3yr not in display cols; skip coloring
        "free_cash_flow_cr": (lambda v, t: v > t, 0),
    },
}
...
def write_sheet(writer, sheet_name, df):
    """Write one preset's screener results to its own colour-coded sheet in the output workbook."""
...
def main():
    """CLI entry point: run all 6 screener presets and write screener_output.xlsx."""
