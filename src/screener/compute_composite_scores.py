...
def compute_composite_for_sector(sector_df):
    ...
    scores["roce_s"] = sector_df["return_on_capital_employed_pct"].apply(
        lambda v: winsorized_score_within_group(
            sector_df["return_on_capital_employed_pct"], v
        )
    )
    ...
    scores["fcf_positive_flag_s"] = (sector_df["free_cash_flow_cr"] > 0).map(
        {True: 100, False: 0}
    )
    ...
    def weighted_row(row):
        """Compute one company's weighted composite score from its individual percentile ranks."""
        parts = [
            (row["roe_s"], 0.15),
            (row["roce_s"], 0.10),
            (row["npm_s"], 0.10),
            (row["fcf_cagr_s"], 0.15),
            (row["cfo_pat_s"], 0.10),
            (row["fcf_positive_flag_s"], 0.05),
            (row["revenue_cagr_s"], 0.10),
            (row["pat_cagr_s"], 0.10),
            (row["de_s"], 0.10),
            (row["icr_s"], 0.05),
        ]
...
def main():
    """CLI entry point: compute sector-relative composite quality scores for all companies."""
    ...
    print(
        f"Computing sector-relative composite scores across {df['broad_sector'].nunique()} sectors..."
    )
    ...
    for _, row in df.iterrows():
        ...
        """,
            (
                row["composite_quality_score_v2"],
                row["fcf_cagr_5yr"],
                row["fcf_cagr_5yr_flag"],
                row["cfo_pat_ratio_5yr"],
                row["company_id"],
                row["year"],
            ),
        )
    ...
    print(
        "(Supersedes Sprint 2's simpler population-wide formula in populate_ratios.py.)"
    )

    sample = (
        df[["company_id", "broad_sector", "composite_quality_score_v2"]]
        .dropna()
        .sample(min(5, len(df)))
    )
    print("\nSample check:")
