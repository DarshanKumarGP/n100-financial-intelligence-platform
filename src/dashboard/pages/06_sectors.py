...
from db import _connect, get_sectors
...
if missing_count > 0:
    st.caption(
        f"{missing_count} of {len(sector_data)} companies in this sector are missing "
...
if plot_data.empty:
    ...
    fig.update_layout(height=500, margin={"t": 30, "b": 10})
...
with col_a:
    ...
    pct_fig.update_layout(height=350, margin={"t": 40, "b": 10}, yaxis_title="Median %")
    st.plotly_chart(pct_fig, use_container_width=True)

with col_b:
    de_fig = px.bar(
        x=["D/E"], y=[full_ratios["debt_to_equity"].median()], title="Debt-to-Equity"
    )
    de_fig.update_layout(height=350, margin={"t": 40, "b": 10}, yaxis_title="Median D/E")
