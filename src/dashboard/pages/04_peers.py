...
from db import _connect, get_peers
...
def normalize(series, value, invert=False):
    """Normalize a metric value onto a common scale for radar chart plotting."""
    valid = series.dropna()
...
if company_row.empty:
    ...
    fig.update_layout(
        polar={"radialaxis": {"visible": True, "range": [0, 100]}}, height=500
    )
...
pivot = peers_long.pivot_table(
    index=["company_id", "company_name", "is_benchmark"],
    columns="metric",
    values="value",
).reset_index()
...
def highlight_benchmark(row):
    """Style the benchmark company's row/trace distinctly in the peer comparison view."""
    ...
    pivot.drop(columns=["is_benchmark"]).style.apply(
        lambda row: [
            (
                ...
                if pivot.loc[row.name, "is_benchmark"] == 1
                else ""
            )
            for _ in row
        ],
