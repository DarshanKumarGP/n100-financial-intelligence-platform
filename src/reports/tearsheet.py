...
wrap_style = ParagraphStyle("wrap", parent=styles["Normal"], fontSize=8, leading=10, wordWrap="CJK")
pro_style = ParagraphStyle("pro", parent=styles["Normal"], fontSize=9, leading=12,
                            textColor=GREEN, wordWrap="CJK")
con_style = ParagraphStyle("con", parent=styles["Normal"], fontSize=9, leading=12,
                            textColor=RED, wordWrap="CJK")
...
def make_revenue_profit_chart(pl):
    ...
    x = range(len(last10))
    width = 0.4
    ax.bar([i - width/2 for i in x], last10["sales"], width, label="Revenue", color="#2a5298")
    ax.bar([i + width/2 for i in x], last10["net_profit"], width, label="Net Profit", color="#4caf50")
...
def make_roe_roce_chart(fr):
    ...
    ax2 = ax1.twinx()  # explicit dual-axis -- confirmed pattern from Sprint 4, never make_subplots(secondary_y=True)

    ax1.plot(last10["year"], last10["return_on_equity_pct"], color="#1a2744", marker="o", label="ROE %")
    ax2.plot(last10["year"], last10["return_on_capital_employed_pct"], color="#c62828", marker="s", label="ROCE %")

    ax1.set_ylabel("ROE %", fontsize=7)
    ax2.set_ylabel("ROCE %", fontsize=7)
    ax1.set_xticks(range(len(last10)))  # pin tick positions before labeling, same pattern as the other 3 charts
...
def make_balance_sheet_stack_chart(bs):
    ...
    ax.bar(x, last10["borrowings"], bottom=last10["equity"], label="Borrowings", color="#c62828")
    bottom2 = last10["equity"] + last10["borrowings"].fillna(0)
    ax.bar(x, last10["other_liabilities"], bottom=bottom2, label="Other Liabilities", color="#9e9e9e")
...
def build_kpi_tiles(fr_latest):
    ...
    label_style = ParagraphStyle("kpi_label", parent=styles["Normal"], fontSize=8, wordWrap="CJK")
    value_style = ParagraphStyle("kpi_value", parent=styles["Normal"], fontSize=13,
...
def build_pros_cons_section(pros, cons):
    elements = []
    elements.append(Paragraph("Pros", ParagraphStyle("pros_head", fontSize=11, textColor=GREEN)))
    if pros:
        for p in pros:
            elements.append(Paragraph(f"&#8226; {p}", pro_style))
    else:
        elements.append(Paragraph("No pros identified above confidence threshold.", wrap_style))

    elements.append(Spacer(1, 6))
    elements.append(Paragraph("Cons", ParagraphStyle("cons_head", fontSize=11, textColor=RED)))
    if cons:
        for c in cons:
            elements.append(Paragraph(f"&#8226; {c}", con_style))
    else:
        elements.append(Paragraph("No cons identified above confidence threshold.", wrap_style))

    return elements


def build_capital_allocation_badge(label):
    badge_style = ParagraphStyle("badge", parent=styles["Normal"], fontSize=10,
                                  textColor=colors.white, alignment=1)
    t = Table([[Paragraph(f"Capital Allocation Pattern: {label}", badge_style)]],
              colWidths=[170 * mm])
    t.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), NAVY),
...
def generate_tearsheet(company_id, output_path):
    ...
    fr, pl, bs, cf, ticker_row = gather_company_data(company_id, conn)
    conn.close()
    ...
    if bs_chart:
        ...
        story.append(Paragraph("Balance sheet composition data not available.", wrap_style))
    story.append(Spacer(1, 6))
...
# ============================================================
# DAY 33 TEST HARNESS -- 5 companies, different sectors
# ============================================================
